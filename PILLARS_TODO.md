...
In flight (agents): **5.2** the Administrator (the last Phases-0–5 build item) · **sim-days harness** (coupled-economy damper verification, required before any 5.5 flag flip). Then: pre-merge capabilities/nuggets sweep + full regression. Note: 0.5 required stopping the live services — see `RUNTIME_SPRINTER.md` for the correct Sprinter procedure.
