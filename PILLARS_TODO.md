...
- Deferred from the review (documented, judged non-blocking): K-sleep quest-abandon path (FAILED close), Administrator-directive visible-tools lint, house-mode+flag interaction guard, flag-off unlock-reward XP leg, `sleeps.total` freeze if mastery_gates ever turns off, loop-side hatch (hatch progress rides the browser's /api/status poll).
