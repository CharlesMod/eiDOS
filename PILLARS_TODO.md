...
- Deferred still open: dashboard panels for roster/missions. (run_post_tick invocation CLOSED 2026-07-10: inline goal-tension/curiosity blocks retired; the registry's post_tick phase is the one dispatch, curiosity's intrinsic rides the ctx to the learner.)

## The tool-unlock ladder + genetics v2 (2026-07-05)

The "far too many tools for an infant AI" arc: the newborn now starts with 8 organs and GROWS the
rest by living. Design docs: **TOOL_PROGRESSION.md** (approved, 6 operator decisions recorded) +
**CREATURE_GENETICS.md** (one germline, many expressions).

- ✅ **Grammar × thinking fix** — GBNF masked gemma-4's think tokens → babble-to-max-tokens; constrained calls now pass `enable_thinking: false` (`cfde7b0`)
- ✅ **Wave 1 foundations** — `unlocks.py` single source of truth (units/grants/adjudicate/seed, `workspace/state/unlocks.json`) (`20a779b`); infant nap curve (egg 0.2 → ~3.6 h bouts ≈ 5 naps/day, stage scale × wake-budget gene) (`2584809`); quest `grants_unit` + honest typed stats (`f69d14e`); genetics v2 germline → morphs/latents/anchors/alleles (`dca0b5d`), seed unity (`ae5b9bc`), `GET /api/phenotype` for the creature-forge (`8e50e21`)
- ✅ **Wave 2 integration** — `visible_tools` accessor, one source five consumers (grammar/prompt/check_tools/manual/dispatch), flag-off returns TOOLS itself byte-identical (`302807e`); prompts decompose into being-text + unit stanzas, lexicon-templated ({mover}/{makers}/{home}) so an otter never says "paws" (`21fe402`)
- ✅ **Capstone gate** — `test_ladder_consistency.py`: grammar, prompt, and mirror must tell one story at 3 ladder points (`6c94ab5`)
- ✅ **Adversarial review → three fix batches** — 45 findings, 10 confirmed: state machines survive crashes/stale seeds/silent settlement (`5fd5dfe`); maiden-boot organs — egg-scale naps for the persona-less newborn, live adenosine ceilings, conception at reset (`dbc7627`); §0 surfaces — mirror derives from visible_tools, sealed unlock rewards ("a new ability"), nuggets 11→8, goal.md/manual reworded, Boss→Charlie (`22e3a78`)
- ✅ **THE FLIP (2026-07-05)** — `tool_unlocks_enabled = true` + the delegate limb wired (`config.local.toml`); fresh slate; **maiden walk verified live**: germline seed 2345144783331623597, **morph otter** (openness 0.89, tenacity −0.25); 8-name universe exact in grammar AND the check_tools mirror; egg nap scale 0.2; coherent ~1 s ticks; unlocks.json `body: born/announced`; genesis ×3 queued; phenotype.json written at birth. First thought: "It's quiet here, and I'm starting to notice the shape of the space around me."
- ✅ **First hour of the walk** — first sleep at ~5 min (a compaction dream counts — infants doze): memory granted on the milestone, genesis-01 issued, skillcraft granted at issuance, the System introduced itself in-world; **senses correctly held PENDING** by the voice-service probe (:8098 down). The deferred Administrator lint **bit immediately** — the first check-in proposed a quest naming `net_scan`/`http_request` at a newborn — so it got built: dossier `creature_tools` + intake hold + approve-time refusal, one visibility source (`9a0647b`).
...
- Deferred from the review (documented, judged non-blocking): house-mode+flag interaction guard, flag-off unlock-reward XP leg, `sleeps.total` freeze if mastery_gates ever turns off. (CLOSED 2026-07-10: **K-sleep quest-abandon** — `quests.abandon_if_stalled`, zero criterion movement across QUEST_STALL_SLEEPS=5 consecutive sleeps closes FAILED at the sleep boundary, `[SYSTEM] QUEST FAILED — STALLED TOO LONG`; **loop-side hatch** — the watchdog's `_hatch_beat` advances cracks/hatch with zero browser polls, same single writer.)
