...
- [ ] 0g. tools misc: tool_http_get + fetch/http aliases, _looks_like_powershell,
      unify tree-killers, unused imports
- [ ] 0h. goal_complete process-exit lifecycle + ask_supervisor/pending_questions
- [ ] 0i. safety/telemetry Linux probes (check_ram/kill_child_processes/get_cpu_temp/
      get_cpu_pct) — Windows equivalents or removal of dead callers
- [ ] 0j. dashboard: /api/pause toggle, legacy WAV speech endpoints, logger NameError
      at ~:404, stale read-only docstring, Pi-era gauges (CPU temp 105C, 64GB disk)
- [ ] 0k. dead config knobs: git_self_branch, llm_local_only, phantom getattr keys
...
- [ ] 0l. rotation gaps: thoughts.jsonl rotation, dream_*.md cap
- [ ] 0m. repo hygiene: exam fixtures (config.json/settings.yaml/defaults.txt) →
      tests/fixtures/, _test_*.py/_analyze.py/_seed_dashboard.py stragglers → scripts/,
      Pi docstrings
- [ ] 0n. doc reconciliation: amend SELF_IMPROVEMENT_PLAN.md (accident-safety re-scope,
